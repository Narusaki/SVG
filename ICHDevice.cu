...
__host__ __device__ ICHDevice::ICHDevice()
{
...
__host__ __device__ void ICHDevice::AssignMesh(Mesh *mesh_)
{
	mesh = mesh_;
}

__host__ __device__ void ICHDevice::AssignBuffers(SplitItem *splitInfos_, unsigned splitInfoSize,
...
__host__ __device__ void ICHDevice::AddSource(unsigned vertId)
{
	sourceVert = vertId;
}

__host__ __device__ void ICHDevice::AddSource(unsigned faceId, Vector3D pos)
{
	sourcePointFace = faceId;
	sourcePointPos = pos;
}

__host__ __device__ void ICHDevice::AddFacesKeptWindow(unsigned faceId)
{
	if (keptFacesIdx == keptFacesSize) return;
	keptFaces[keptFacesIdx++] = faceId;
}

__host__ __device__ void ICHDevice::Execute(int totalCalcVertNum_)
{
	// Initialize
	Initialize();
...
__host__ __device__ void ICHDevice::OutputStatisticInfo()
{
	/*
...
__host__ __device__ double ICHDevice::BuildGeodesicPathTo(unsigned faceId, Vector3D pos, unsigned &srcId,
	unsigned &nextToSrcEdge, double &nextToSrcX, unsigned &nextToDstEdge, double &nextToDstX)
{
	...
	else
	{
		...
		else if (!vertInfos.get(opVert).isSource)
		{
			gkp.isVertex = true;
			gkp.id = opVert;
...
__host__ __device__ void ICHDevice::BuildGeodesicPathTo(unsigned vertId, unsigned &srcId,
	unsigned &nextToSrcEdge, double &nextToSrcX, unsigned &nextToDstEdge, double &nextToDstX)
{
	...
	while (vertInfos.get(curVert).dist != 0.0)
	{
		...
		else
		{
			...
			if (!vertInfos.get(opVert).isSource)
			{
				gkp.isVertex = true;
				gkp.id = opVert;
				nextToSrcEdge = mesh->verts[gkp.id].firstEdge; nextToSrcX = 0.0;
...
__host__ __device__ double ICHDevice::GetDistanceTo(unsigned vertId)
{
	return vertInfos.get(vertId).dist;
}

__host__ __device__ void ICHDevice::Clear()
{
	...
	for (int i = 0; i < size + 1; ++i)
	{
		...
		vertData[i].item.pseudoSrcId = -1;
		vertData[i].item.srcId = -1;
		vertData[i].index = -1;
	}
	sourceVert = -1;
	sourcePointFace = -1;
...
__host__ __device__ void ICHDevice::Initialize()
{
	if (sourceVert != -1)
	{
		...
		do
		{
			...
			if (mesh->edges[curEdge].edgeLen > vertInfos[opVert].dist)
			{
			...
			vertInfos[opVert].srcId = sourceVert; vertInfos[opVert].pseudoSrcId = sourceVert;
		...
		vertInfos[sourceVert].srcId = sourceVert; vertInfos[sourceVert].pseudoSrcId = sourceVert;
	}
	...
	if (sourcePointFace != -1)
	{
		for (int j = 0; j < 3; ++j)
		{
			...
			if ((sourcePointPos - mesh->verts[opVert].pos).length() > vertInfos[opVert].dist) continue;
			vertInfos[opVert].birthTime = 0;
			vertInfos[opVert].dist = (sourcePointPos - mesh->verts[opVert].pos).length();
			vertInfos[opVert].enterEdge = -1;
			vertInfos[opVert].srcId = mesh->vertNum; vertInfos[opVert].pseudoSrcId = mesh->vertNum;
...
__host__ __device__ void ICHDevice::PropagateWindow(const Window &win)
{
...
__host__ __device__ void ICHDevice::GenSubWinsForPseudoSrc(const PseudoWindow &pseudoWin)
{
...
__host__ __device__ void ICHDevice::GenSubWinsForPseudoSrcFromWindow(const PseudoWindow &pseudoWin, unsigned &startEdge, unsigned &endEdge)
{
...
__host__ __device__ void ICHDevice::GenSubWinsForPseudoSrcFromPseudoSrc(const PseudoWindow &pseudoWin, unsigned &startEdge, unsigned &endEdge)
{
	unsigned pseudoSrc = pseudoWin.vertID;
...
__host__ __device__ bool ICHDevice::IsValidWindow(const Window &win, bool isLeftChild)
{
...
__host__ __device__ void ICHDevice::BuildWindow(const Window &fatherWin,
	unsigned edge,
	double t0, double t1,
...
__host__ __device__ double ICHDevice::Intersect(const Vector2D &v0, const Vector2D &v1, const Vector2D &p0, const Vector2D &p1)
{
